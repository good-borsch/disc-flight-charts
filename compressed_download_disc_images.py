...
- Resizes to 300x300 and converts to optimized JPEG for mobile display
...
def download_and_convert_image(img_url: str) -> bytes | None:
    """Download image, resize to 150x150, and convert to optimized JPEG."""
    headers = {"User-Agent": USER_AGENT}
    ...
    try:
        ...
        # Convert to RGB (required for JPEG - removes alpha channel)
        if img.mode != "RGB":
        ...
        # Resize to 300x300 using high-quality downsampling
        ...
        img.save(
            jpeg_buffer,
            format="JPEG",
            quality=82,          # Good balance of quality/size for mobile
            optimize=True,       # Optimize Huffman tables
            progressive=True,    # Progressive loading (better perceived performance)
        )
...
def process_disc(disc: dict) -> tuple[int, bytes | None, str | None]:
    ...
    Returns: (disc_id, img_bytes, filename)
    """
    disc_id = disc["id"]
    ...
    filename = f"{sanitize_filename(manufacturer)}_{sanitize_filename(model)}.jpg"
    ...
    img_bytes = download_and_convert_image(img_url)
    if not img_bytes:
        return (disc_id, None, None)
    
    logger.info(f"Successfully processed: {manufacturer} {model} ({len(img_bytes)} bytes)")
    return (disc_id, img_bytes, filename)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_disc):
            ...
            try:
                disc_id, img_bytes, filename = future.result()
                
                if img_bytes and filename:
                    # Save to database
                    update_db_image(DB_PATH, disc_id, img_bytes)
                    
                    # Save to file as backup
                    save_to_file(filename, img_bytes)
